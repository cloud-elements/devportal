...
* __Example: select id,name from accounts where name=’foo’ order by name asc limit 10 offset 0__
...
CEQL, or at least parts of CEQL, can be used in most of our find or search related APIs. You can add CEQL as query parameters to API calls (for example, `GET accounts?where=Name='Foo'`) or directly in our API Documentation as shown below.
