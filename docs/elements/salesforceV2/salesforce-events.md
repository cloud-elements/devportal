...
Cloud Elements currently supports events via <a href="#" data-toggle="tooltip" data-original-title="{{site.data.glossary.polling}}">polling</a> or <a href="#" data-toggle="tooltip" data-original-title="{{site.data.glossary.webhooks}}">webhooks</a> depending on the endpoint.

{% include note.html content="Cloud Elements normalizes only the objectId, objectType, and eventType event data. Event data also contains raw data, which the service provider returns. The raw data varies based on the service provider." %}
